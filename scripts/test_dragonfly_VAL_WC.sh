bash -c '> plot/dragonfly_latency_VAL_WC.txt'
bash -c '> plot/dragonfly_reception_rate_VAL_WC.txt'
bash -c '> plot/dragonfly_hops_VAL_WC.txt'
...
do
  ...
  echo "injection_rate = $injection_rate  average_packet_latency = $(echo "scale=6; $(grep "average_packet_latency" m5out/stats.txt | awk '{ print $2 }') / 1000" | bc)  (Cycle/Packet)" >> plot/dragonfly_latency_VAL_WC.txt
  ...
  echo -e "injection_rate = $injection_rate  reception_rate = $reception_rate  (Packets/Node/Cycle)" >> plot/dragonfly_reception_rate_VAL_WC.txt
  echo "injection_rate = $injection_rate  average_hops = $(grep "average_hops" m5out/stats.txt | awk '{ print $2 }')  (Cycle/Packet)" >> plot/dragonfly_hops_VAL_WC.txt
