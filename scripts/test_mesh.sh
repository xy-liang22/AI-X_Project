...
do
  ...
  ./build/NULL/gem5.opt configs/example/garnet_synth_traffic.py --network=garnet --num-cpus=36 --num-dirs=64 --topology=Mesh_XY --mesh-rows=6 --routing-algorithm=1  --inj-vnet=0 --synthetic=uniform_random --garnet-deadlock-threshold=50000 --sim-cycles=100000000 --injectionrate=$injection_rate
  ...
  echo "injection_rate = $injection_rate  average_hops = $(grep "average_hops" m5out/stats.txt | awk '{ print $2 }')  (Cycle/Packet)" >> plot/mesh_hops.txt
